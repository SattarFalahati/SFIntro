...
+ (instancetype)initWithFrame:(CGRect)frame withParallaxBackgroundImage:(NSString *)strParallaxBGImage withDelegate:(id<SFIntroDelegate>) delegate withDataSourceArray:(NSArray *)arrDataSource
{
    ...
    if (arrDataSource.count == 0) return nil;
...
- (void)initialPager
{
    ...
    self.pager.currentPageIndicatorTintColor = [UIColor blueColor]; // This can be change by user
    self.pager.pageIndicatorTintColor = [UIColor redColor]; // This can be change by user
}
